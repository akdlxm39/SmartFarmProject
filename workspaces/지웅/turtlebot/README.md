...
8. 프로젝트 확장용 `slam_pjt` ROS2 package와 TurtleBot Modbus heartbeat/status smoke client 준비
...
Notion 관통 프로젝트 메모에 따라 `slam_pjt` package를 준비했다.

```bash
cd /home/ssafy/work/SmartFarmProject/workspaces/지웅/turtlebot/ros2_ws
colcon build --symlink-install --packages-select slam_pjt
...
```bash
ros2 run slam_pjt turtlebot_modbus_heartbeat --once --dry-run
```

실제 Modbus server write 예시:

```bash
ros2 run slam_pjt turtlebot_modbus_heartbeat --host 192.168.110.109 --port 50200
```
