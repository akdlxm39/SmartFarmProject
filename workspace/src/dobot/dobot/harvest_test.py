...
try:
    from dobot.vision_capture_client import CaptureResult, VisionCaptureClient
...
def find_package_root_from_file() -> Optional[Path]:
    ...
    for parent in here.parents:
        if parent.name == "dobot" and (parent / "package.xml").exists():
            return parent
    return None
...
def default_calibration_path() -> Path:
    ...
    # Normal operator flow: cd .../ros2_ws && ros2 run dobot harvest_test
    source_config = cwd / "src" / "dobot" / "config" / "dobot_positions_latest.json"
    if source_config.exists():
        return source_config
    ...
    try:
        ...
        share_config = (
            Path(get_package_share_directory("dobot"))
            / "config"
