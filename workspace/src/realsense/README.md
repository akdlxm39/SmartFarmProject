...
colcon build --packages-select realsense
...
```bash
ros2 run realsense topview_color_detector
```

창이 너무 크면:

```bash
ros2 run realsense topview_color_detector --ros-args \
  -p display_scale:=0.7
```
...
```bash
ros2 run realsense conveyor_modbus_command emergency_stop
ros2 run realsense conveyor_modbus_command reset
ros2 run realsense conveyor_modbus_command run_clockwise --speed 100
ros2 run realsense conveyor_modbus_command run_counter_clockwise --speed 100
ros2 run realsense conveyor_modbus_command stop
```
